...
most common one, and the time slider opens on the flight proper rather than on
the whole log. See section~\ref{sec:window} for why.

\begin{figure}[htbp]
\centering
\screenshot{screen-window.png}{0.95\textwidth}
  {The whole window with the Day 5 log loaded.}
\caption{The application window, Day 5 loaded and tag 42 selected. The control
column is on the left in four groups; the plot, the toolbar and the two range
sliders each get their own strip on the right. The dashed arrow is the current
bearing estimate, and the same three numbers appear under \textbf{Bearing} in
the control column and in the plot title. Both implementations use this
arrangement and these four groups.}
\end{figure}

\subsection{Takeoff and landing are deselected to start with}
\label{sec:window}

A flight begins and ends at the ground. Pulses received during the climb and the
descent are weaker than the rest --- on the Day 8 flight they average 24 to
27\,dB against 42.9\,dB at working altitude --- and they are all clustered
around the launch point, so they pull the interpolated surface down in one
corner of the map and make the site look quieter than it is.

The application therefore opens the time slider on the flight proper. The rule
is the altitude: a survey climbs to a working height and holds it, so the median
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.49\textwidth]{plot-day8-with-takeoff.pdf}\hfill
\includegraphics[width=0.49\textwidth]{plot-day8-cruise.pdf}
\caption{Day 8, tag 12. Left: the whole log, as earlier versions opened it. The
blue patch at the origin is takeoff and landing, and it drags the surface and
the colour scale down around the launch point. Right: what you now get on load,
with the climb and descent deselected --- 307 of the 460 pulses, and a surface
that reflects the survey rather than the ground.}
\label{fig:window}
\end{figure}

\noindent \textbf{Nothing is discarded.} The slider's limits still cover the
whole log; only its handles start inside. Drag the left handle back to zero and
the takeoff reappears. If the altitudes cannot support the judgement --- all
equal, all missing, or a window so short that the altitude is more likely to be
junk than the flight to have been that brief --- the slider opens on the whole
log as before.
...
\caption{Day 5, tag 42 (left) and tag 40 (right) from the same flight, on the
default time window: 434 and 429 pulses respectively. Grey dots are all 1111
pulses in the file, including the tags not selected and the takeoff and landing
the window excludes; coloured dots are the selection; the filled surface is SNR
interpolated over it. The two tags agree on a bearing to within a degree. Real
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{screen-controls.png}
\caption{The control column, shown as two halves side by side so it fits the
page. Values are from the Day 5 flight. The four groups are \textbf{Data
selection}, \textbf{Surface}, \textbf{Tag position} and \textbf{Bearing}, in
that order down the column.}
\end{figure}
...
Time / SNR sliders & Lower and upper bounds. Replot on release. The time slider
opens on the flight proper, with takeoff and landing deselected but still within
its limits --- see section~\ref{sec:window}. \\
\bottomrule
\end{tabular}
\end{center}
...
and cancel; the resultant still has a direction but $R=0.15$ says not to trust
...
\textbf{Altitude (m)} and the time slider widened to the whole log: the surface
is the aircraft's own height profile, so the field agrees strongly ($R=0.88$,
the highest number in this guide) on a bearing of $83.6^\circ$ that has nothing
whatever to do with the tag. A high $R$ means the cells agree, not that you
asked a sensible question. (On the default window the aircraft holds one
altitude, so there is no surface at all and the bearing reads \code{-}.)}
\end{figure}
...
Fewer pulses than the file contains, on load &
  Expected: takeoff and landing start deselected. Widen the time slider to see them. \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{quote}\code{make -C DOCS all}\end{quote}

\noindent That runs \code{make\_figures.py} for the plots,
\code{make\_screenshots.py} for the two screenshots, and \code{latexmk} for the
document. The two pulse logs live outside the repository, under
\code{FLIGHT\_TESTING\_DATA}; pass other paths through with
\code{make figures LOGS="day5.csv day8.csv"}.

The screenshots are macOS-only and need Screen Recording permission for whatever
runs them, under \textbf{System Settings $\rightarrow$ Privacy \& Security
$\rightarrow$ Screen \& System Audio Recording}. The window must also fit on the
main display; widen \code{WINDOW} in \code{make\_screenshots.py} if yours is
bigger than the 1024\,px it currently asks for.
